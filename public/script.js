...
async function loadProducts() {
    try { products = await api('/products'); renderProducts(); }
...
function renderProducts() {
    ...
    if (products.length === 0) {
        productGrid.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:#888;">Товаров пока нет</p>';
        return;
    }
...
function openProduct(id) {
    ...
    document.getElementById('pv-price').textContent = currentProduct.price.toLocaleString('ru-RU') + ' ₽';
    ...
    if (currentProduct.brand) {
        ...
        brandEl.onclick = () => showBrand(currentProduct.brand.id);
        brandEl.style.display = 'block';
    } else {
        brandEl.style.display = 'none';
    }
    renderProductGallery();
    document.getElementById('pv-sizes').innerHTML = (currentProduct.sizes || []).map(size =>
        `<button class="size-btn ${size === currentSize ? 'selected' : ''}" onclick="selectSize('${size}',this)">${size}</button>`
    ).join('');
    hideAllViews();
...
function renderProductGallery() {
    ...
    document.querySelectorAll('.gallery-arrow').forEach(a => a.style.display = currentProductImages.length > 1 ? 'block' : 'none');
    thumbs.innerHTML = currentProductImages.length > 1
        ? currentProductImages.map((src, i) => `<img class="thumb ${i === currentImageIndex ? 'active' : ''}" src="${src}" onclick="goToProductImage(${i})" alt="">`).join('')
        : '';
}
...
function addCurrentToCart() {
    ...
    if (existing) {
        ...
        cart.push({
            ...
            price: currentProduct.price, size: currentSize, qty: currentQty,
...
function renderCart() {
    ...
    box.innerHTML = `
        ...
        <div class="cart-form">
            ...
            <div class="form-group"><label>Имя и фамилия *</label><input type="text" id="o-name"></div>
            <div class="form-group"><label>Телефон *</label><input type="text" id="o-phone"></div>
            <div class="form-group"><label>Адрес доставки *</label><input type="text" id="o-address"></div>
            <div class="form-group"><label>Комментарий</label><textarea id="o-comment"></textarea></div>
            <button class="add-to-cart-btn" onclick="checkout()">Оплатить</button>
        </div>
    `;
}
    ...
    try {
        await api('/orders', {
            ...
            body: JSON.stringify({
                ...
                items: cart.map(i => ({ id: i.id, name: i.name, price: i.price, size: i.size, qty: i.qty })),
                total
            })
        });
        cart = [];
...
function renderBrands() {
    ...
    grid.innerHTML = brands.map(b => `
        <div class="brand-card" onclick="showBrand(${b.id})">
...
async function loadContacts() {
    try { contacts = await api('/contacts'); } catch { contacts = {}; }
...
async function checkAuth() {
    ...
    catch { authToken = ''; localStorage.removeItem('vs_token'); currentUser = null; }
    renderUserArea();
}
...
async function doRegister() {
    ...
    try {
        ...
        authToken = data.token; localStorage.setItem('vs_token', authToken);
        currentUser = data.user; closeAuthModal(); renderUserArea();
...
async function doLogin() {
    ...
    try {
        ...
        authToken = data.token; localStorage.setItem('vs_token', authToken);
        currentUser = data.user; closeAuthModal(); renderUserArea();
        alert('С возвращением, ' + currentUser.name + '!');
    } catch (e) { errorEl.textContent = e.message; }
}
function doLogout() {
    authToken = ''; localStorage.removeItem('vs_token');
    currentUser = null; renderUserArea(); showCatalog();
}
...
async function openAdmin() {
    if (!currentUser || currentUser.role !== 'admin') { alert('Только для админа'); return; }
    hideAllViews();
    adminView.classList.add('active');
    try { await loadProducts(); renderAdminProducts(); } catch {}
    try { await loadBrands(); renderAdminBrands(); fillBrandSelect(); } catch {}
    try { await loadUsers(); renderAdminUsers(); } catch {}
    try { await loadSlides(); renderAdminSlides(); } catch {}
    try { await loadOrders(); renderAdminOrders(); } catch {}
    try { await loadContacts(); fillContactsForm(); } catch {}
    window.scrollTo({ top: 0, behavior: 'smooth' });
}
...
async function loadUsers() { try { users = await api('/users'); } catch { users = []; } }
...
async function handleFileUpload(files) {
    ...
    list.forEach(file => {
        ...
        reader.onload = (e) => { currentPhotos.push({ local: e.target.result, file }); renderProductPhotos(); };
    ...
    try {
        ...
        let i = 0;
        currentPhotos = currentPhotos.map(p => p.local && p.file ? data.urls[i++] : p);
        renderProductPhotos();
    } catch (e) {
        alert('Ошибка загрузки: ' + e.message);
        currentPhotos = currentPhotos.filter(p => typeof p === 'string');
        renderProductPhotos();
    }
}
...
function renderProductPhotos() {
    ...
    list.innerHTML = currentPhotos.map((photo, i) => {
...
function makeMainPhoto(i) {
    if (i === 0) return;
    const [p] = currentPhotos.splice(i, 1);
...
async function saveProduct() {
    ...
    const images = currentPhotos.map(p => typeof p === 'string' ? p : p.local);
    if (images.length === 0) { alert('Добавьте фото'); return; }
    const sizes = sizesStr ? sizesStr.split(',').map(s => s.trim()).filter(Boolean) : ['ONE SIZE'];
    const payload = { name, price, description: desc, sizes, images, brand_id: brand_id ? parseInt(brand_id) : null };
    try {
        if (id) { await api('/products/' + id, { method: 'PUT', body: JSON.stringify(payload) }); alert('Товар обновлён!'); }
        else { await api('/products', { method: 'POST', body: JSON.stringify(payload) }); alert('Товар добавлен!'); }
        resetProductForm();
...
function resetProductForm() {
    ['edit-id','f-name','f-price','f-desc','f-sizes','f-img-url'].forEach(i => {
...
async function saveBrand() {
    ...
    if (!name) { alert('Введите название бренда'); return; }
    try {
        await api('/brands', { method: 'POST', body: JSON.stringify({ name, description, logo }) });
...
function renderSlider() {
    ...
    if (dotsContainer) dotsContainer.innerHTML = slides.map((_, i) =>
        `<span class="dot ${i === currentSlide ? 'active' : ''}" onclick="goToSlide(${i})"></span>`).join('');
...
function goToSlide(i) {
    ...
    document.querySelectorAll('#slider-dots .dot').forEach((d, idx) => d.classList.toggle('active', idx === i));
}
...
function renderAdminSlides() {
    ...
    list.innerHTML = slides.map(s => `<div class="slide-item"><img src="${s.img}" alt=""><button class="slide-remove" onclick="removeSlide(${s.id})">×</button></div>`).join('');
}
async function addSlide() {
    const url = document.getElementById('slide-url').value.trim();
    const img = newSlideData || url;
    if (!img) { alert('Добавьте фото'); return; }
    try {
        await api('/slides', { method: 'POST', body: JSON.stringify({ img }) });
        newSlideData = '';
...
/* ============================================================
   АДМИНКА: ЗАКАЗЫ
   ============================================================ */
async function loadOrders() { try { orders = await api('/orders'); } catch { orders = []; } }
function renderAdminOrders() {
    ...
    tbody.innerHTML = orders.map(o => `
        <tr>
            ...
            <td>${o.phone}</td>
            <td>${o.address}</td>
            <td>${o.items.map(i => `${i.name} (${i.size}) ×${i.qty}`).join('<br>')}</td>
...
async function updateOrderStatus(id, status) {
    try { await api('/orders/' + id, { method: 'PUT', body: JSON.stringify({ status }) }); }
    catch (e) { alert('Ошибка: ' + e.message); }
}
...
async function saveContacts() {
    ...
    try { contacts = await api('/contacts', { method: 'PUT', body: JSON.stringify(payload) }); alert('Контакты сохранены!'); }
    catch (e) { alert('Ошибка: ' + e.message); }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (uz && fi) {
        ...
        fi.addEventListener('change', (e) => { handleFileUpload(e.target.files); fi.value = ''; });
        uz.addEventListener('dragover', (e) => { e.preventDefault(); uz.classList.add('dragover'); });
        uz.addEventListener('dragleave', () => uz.classList.remove('dragover'));
        uz.addEventListener('drop', (e) => { e.preventDefault(); uz.classList.remove('dragover'); handleFileUpload(e.dataTransfer.files); });
    }
    ...
    if (bz && bf) {
        bz.addEventListener('click', () => bf.click());
        bf.addEventListener('change', async (e) => {
    ...
    if (sz && sf) {
        sz.addEventListener('click', () => sf.click());
        sf.addEventListener('change', (e) => uploadSlideFile(e.target.files[0]));
        ...
        sz.addEventListener('drop', (e) => { e.preventDefault(); sz.classList.remove('dragover'); uploadSlideFile(e.dataTransfer.files[0]); });
    }
